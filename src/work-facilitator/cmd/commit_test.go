...
func TestCommitCommand_SkipPreCommit(t *testing.T) {
	...
	// This test verifies that when skipPreCommitArg is true,
	// the hook execution is bypassed
	t.Run("skip pre-commit with -s flag", func(t *testing.T) {
		// Reset the flag
		skipPreCommitArg = false

		// Parse flags to set skipPreCommitArg
		cmd := &cobra.Command{}
		cmd.Flags().BoolVarP(&skipPreCommitArg, "skip-precommit", "s", false, "Skip pre-commit hooks")
		cmd.ParseFlags([]string{"-s"})

		// Verify flag was set
		if !skipPreCommitArg {
			t.Error("Expected skipPreCommitArg to be true when -s flag is passed")
		}
	})
	...
	t.Run("run pre-commit without -s flag", func(t *testing.T) {
		...
		if skipPreCommitArg {
			t.Error("Expected skipPreCommitArg to be false when -s flag is not passed")
		}
	})
}
